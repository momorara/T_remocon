...
air_set03 -> remocon_mode03_02.py-> remocon_mode03_03.py -> remocon_noTemp_01.py
 -> remocon_noTemp_02.py  -> remocon_noTemp_03.py
...
2022/06/18  トイレ洗浄用
    01      リモコンだけではなくスイッチで運用できるようにしていく
            iR_command 改行を削除
            swでip_check_OLED.py起動 ただし、一度でもirrpを動かすとswを受け付けなくなる
...
def main():
    ...
    disp_mes_list = ['モードを選択','1:運用',        '9:終了','選んでください']
    while True:
        for disp_mes in disp_mes_list:
            ...
            if cmd != 'no_cmd':
                break
            # sw読み取り
            # ただし、irrpを起動するとswを受け付けなくなる。
            # まぁip関係は一度wifiに加入してしまえば、以後不要ですけどね。
            # print(Readsw())
            if Readsw() == 'on':
                go_ip = 'python3 ' + path + 'ip_check_OLED.py'
                subprocess.run(go_ip, shell=True)
