...
2022/06/18  トイレ水洗用に改造
    04      リモコンコマンド以外でもiR信号を受信したら
            水洗間隔のリセットを行う。
2022/06/23  wash countdown フルパスで指定
...
scp -r T_remocon/irrp_data_code03.py pi@192.168.68.128:/home/pi/T_remocon
scp -r T_remocon pi@192.168.68.126:/home/pi
...
def print_message():
    log_print ('|********************************|')
    log_print ('| irrp_data_code04.py      2     |')
    log_print ('|********************************|')
...
# -----------------------------
      ...
      # コードの受信を確認したので、洗浄があったと判断し、wash_countdown.txtをリセットする
      # wash_time_minut.txt 洗浄間隔を分にしたファィル 変化しない
      # wash_countdown.txt  洗浄間隔を分にしたファィル カウントダウンしていく
      ...
      try:
        log_print('wash countdown reset')
        shutil.copyfile(path + "wash_time_minut.txt", path + "wash_countdown.txt")
      except:
        log_print('wash countdown file not found')
        pass
      # -------------------------------------------
      try:
        # なぜかたまにここでエラーになることがある
